...
import { Code, Clock, Cpu, Terminal, CheckCircle, Maximize2, Zap } from "lucide-react";
import VisualizerLayout from "../../components/VisualizerLayout";
import { useVisualizer } from "../../hooks/useVisualizer";

const FindMaxElement = () => {
  ...
  const handleLoad = useCallback((customArr) => {
    ...
    newHistory.push({
      ...
      maxIndex: 0,
      comparisons: 0,
      message: `Initialize max_index = 0 (value: ${arr[0]}).`,
      line: 2,
    });
    ...
    for (let i = 1; i < arr.length; i++) {
      ...
      newHistory.push({
        ...
        maxIndex,
        comparisons,
        message: `Compare arr[${i}] (${arr[i]}) with current max arr[${maxIndex}] (${arr[maxIndex]}).`,
        line: 4,
      });
      ...
      if (arr[i] > arr[maxIndex]) {
        ...
        newHistory.push({
          ...
          maxIndex,
          comparisons,
          message: `arr[${i}] (${arr[i]}) is greater. Update max_index = ${i}.`,
          line: 5,
        });
      }
    }
    ...
    newHistory.push({
      ...
      maxIndex,
      comparisons,
      message: `Loop finished. Return maximum element arr[${maxIndex}] = ${arr[maxIndex]}.`,
      line: 8,
    });
  ...
  const codeContent = {
    1: `int findMax(vector<int>& arr) {`,
    2: `    int max_index = 0;`,
    3: `    for (int i = 1; i < arr.size(); i++) {`,
    4: `        if (arr[i] > arr[max_index]) {`,
    5: `            max_index = i;`,
    6: `        }`,
    7: `    }`,
    8: `    return arr[max_index];`,
    9: `}`
  };
    ...
    <>
      <div className="p-4 text-center bg-gray-900/50 backdrop-blur-md rounded-2xl border border-gray-700/60 shadow-2xl">
        ...
        <div className="text-3xl font-mono text-yellow-300">
          i={currentIndex < displayArray.length ? currentIndex : "-"} | max={maxIndex}
        </div>
      </div>
      ...
      <div className="p-4 text-center bg-gray-900/50 backdrop-blur-md rounded-2xl border border-gray-700/60 shadow-2xl">
        <h4 className="font-semibold flex items-center justify-center gap-2 mb-2 text-emerald-300 select-none text-sm">
          <CheckCircle size={14} /> Maximum Value
        </h4>
      ...
      <div className="sm:col-span-3 p-4 bg-gray-900/50 backdrop-blur-md rounded-2xl border border-gray-700/60 shadow-2xl">
        ...
        <div className="text-xs text-gray-300 space-y-1">
          ...
          <div>
            <strong>Space Complexity:</strong> <span className="font-mono text-cyan-300">O(1)</span> — Only uses a pointer to track the maximum element.
          </div>
        </div>
      </div>
    </>
  ...
  return (
    <VisualizerLayout
      ...
      description="Visualizing the process of finding the largest element in an array."
      isLoaded={isLoaded}
                ...
                >
                  ...
                  <div
                    className={`w-full text-center py-1 text-xs font-bold rounded-b-md ${
                      isMax ? "bg-green-500 text-white font-black" : "bg-gray-700 text-gray-400"
                    }`}
                  >
                    {isMax ? "MAX" : " "}
                  </div>
